...
const LeaveCalendarView: React.FC = () => {
  ...
  const scrollToCurrentWeek = useCallback(() => {
    ...
    useLayoutEffect(() => {
      ...
      }, [visibleWeeks, scrollToCurrentWeek]);
  ...
  const WeekViewComponent: React.FC<WeekViewProps> = ({
      ...
      return (
          <section className="w-full" data-current={isCurrent}>
              <div ref={weekHeaderRef} className={`week-header sticky top-40 z-20 bg-white bg-opacity-95 border-b border-gray-200 py-2 px-4 font-bold text-gray-800 transition-all duration-200 ${isCurrent ? 'bg-blue-50' : ''}`}>
              ...
              <div className="grid grid-cols-7 gap-px border-b border-gray-200">
                  {calendarDays.map((currentDate) => {
                      ...
                      return (
                          <div key={currentDate.toDateString()} className={`min-h-[85px] p-2 relative border border-gray-100 ${isToday ? 'bg-blue-50' : isWeekend ? 'bg-gray-50' : 'bg-white'}`}>
                              <div className={`text-sm mb-1 ${isToday ? 'bg-blue-500 text-white rounded-full w-6 h-6 flex items-center justify-center' : ''}`}>
                                  {day}
                              </div>
                              <div className="space-y-1 overflow-y-auto max-h-[80%]">
                                  {dayLeaveRequests.map((leaveRequest) => (
                                      <LeaveItem key={`${leaveRequest.id}-${day}`} leaveRequest={leaveRequest} isStart={isLeaveStartDate(currentDate, leaveRequest)} isEnd={isLeaveEndDate(currentDate, leaveRequest)} />
                                  ))}
                              </div>
                          </div>
                      );

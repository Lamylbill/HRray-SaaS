...
export const useRecentActivities = (initialPage = 1, initialPageSize = 10, initialCategory: ActivityCategory = 'all') => {
  ...
  const fetchActivities = async () => {
    ...
    try {
      ...
      const { count: activitiesCount, error: countError } = await countQuery;
      
      if (countError) throw countError;
      
      setTotalCount(activitiesCount || 0);

...
const plans = [
  {
    name: "Basic",
    price: "Free",
    description: "For micro teams needing simple HR.",
    features: [
      "Employee records",
      "Leave management",
      "Payroll export (CSV)",
  ...
  },
  {
    name: "Pro",
    price: "S$19/mo",
    description: "For growing SMEs (most popular)",
    features: [
      "All Basic features",
  ...
  },
  {
    name: "Enterprise",
    price: "S$49/mo",
    description: "For teams needing customization",
    features: [
      "All Pro features",
      "Custom workflows",
  ...
  return (
    <>
      ...
      <div className="flex flex-col min-h-screen bg-gradient-blue">
        ...
        <section className="py-28 px-6 bg-blue-50" id="pricing-table">
          <div className="container mx-auto max-w-7xl">
            <div className="text-center mb-12">
              <h2 className="text-4xl font-bold text-blue-950 mb-4">Compare our Plans & Pricing</h2>
              <p className="text-blue-800 text-xl">Clear, affordable subscriptions for every SME size</p>
            </div>
            <div className="grid gap-10 md:grid-cols-3">
              {plans.map((plan, idx) => (
                <div
                  key={plan.name}
                  className={[
                    "rounded-3xl shadow-lg border py-10 px-8 flex flex-col items-center bg-white relative",
                    plan.highlight ? "border-blue-600 shadow-2xl scale-105 z-10" : "border-gray-200"
                  ].join(" ")}
                >
                  {plan.badge && plan.highlight && (
                    <div className="absolute -top-5 left-1/2 -translate-x-1/2 bg-blue-600 text-white px-4 py-1 text-xs font-bold rounded-full">
                      {plan.badge}
                    </div>
                  )}
                  ...
                  <ul className="mb-8 flex-1 space-y-3 w-full text-left">
                    {plan.features.map((f, i) => (
                      <li key={i} className="flex items-center text-blue-800">
                        <svg className="w-5 h-5 mr-2 text-blue-600" fill="currentColor" viewBox="0 0 20 20"><path d="M16.707 6.293a1 1 0 010 1.414l-7.071 7.071a1 1 0 01-1.414 0l-3.536-3.535a1 1 0 111.414-1.415l2.828 2.829 6.364-6.364a1 1 0 011.415 0z"></path></svg>
                        {f}
                      </li>
                    ))}
                  </ul>
                  {plan.name === "Basic" && (
                    <a href="#contact">
                      <Button size="lg" className="w-full rounded-full bg-blue-600 text-white shadow-button">
                        {plan.cta}
                      </Button>
                    </a>
                  )}
                  {plan.name === "Pro" && (
                    <a href="#contact">
                      <Button size="lg" className="w-full rounded-full bg-indigo-700 text-white hover:bg-indigo-800">
                        {plan.cta}
                      </Button>
                    </a>
                  )}
                  {plan.name === "Enterprise" && (
                    <a href="#contact">
                      <Button variant="outline" size="lg" className="w-full rounded-full border-blue-200">
                        {plan.cta}
                      </Button>
                    </a>
                  )}
            ...
            <p className="text-blue-700 mt-8 text-center text-sm">All paid plans include a free 14-day trial. No credit card required to start.</p>
          </div>
        </section>
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-10">
              <div className="col-span-2">
                ...
                <div className="flex space-x-4">
                  <a href="#" aria-label="Go to Facebook" className="text-blue-600 hover:text-blue-800 transition-colors">
                    <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                      <path fillRule="evenodd" d="M22 12c0-5.523-4.477-10-10-10S2 6.477 2 12c0 4.991 3.657 9.128 8.438 9.878v-6.987h-2.54V12h2.54V9.797c0-2.506 1.492-3.89 3.777-3.89 1.094 0 2.238.195 2.238.195v2.46h-1.26c-1.243 0-1.63.771-1.63 1.562V12h2.773l-.443 2.89h-2.33v6.988C18.343 21.128 22 16.991 22 12z" clipRule="evenodd"></path>
                    </svg>
                  </a>
                  <a href="#" aria-label="Go to twitter" className="text-blue-600 hover:text-blue-800 transition-colors">
                    <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                      <path d="M8.29 20.251c7.547 0 11.675-6.253 11.675-11.675 0-.178 0-.355-.012-.53A8.348 8.348 0 0022 5.92a8.19 8.19 0 01-2.357.646 4.118 4.118 0 001.804-2.27 8.224 8.224 0 01-2.605.996 4.107 4.107 0 00-6.993 3.743 11.65 11.65 0 01-8.457-4.287 4.106 4.106 0 001.27 5.477A4.072 4.072 0 012.8 9.713v.052a4.105 4.105 0 003.292 4.022 4.095 4.095 0 01-1.853.07 4.108 4.108 0 003.834 2.85A8.233 8.233 0 012 18.407a11.616 11.616 0 006.29 1.84"></path>
                    </svg>
                  </a>
                  <a href="#" aria-label="Go to Instagram" className="text-blue-600 hover:text-blue-800 transition-colors">
                    <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                      <path fillRule="evenodd" d="M12.315 2c2.43 0 2.784.013 3.808.06 1.064.049 1.791.218 2.427.465a4.902 4.902 0 011.772 1.153 4.902 4.902 0 011.153 1.772c.247.636.416 1.363.465 2.427.048 1.067.06 1.407.06 4.123v.08c0 2.643-.012 2.987-.06 4.043-.049 1.064-.218 1.791-.465 2.427a4.902 4.902 0 01-1.153 1.772 4.902 4.902 0 01-1.772 1.153c-.636.247-1.363.416-2.427.465-1.067.048-1.407.06-4.123.06h-.08c-2.643 0-2.987-.012-4.043-.06-1.064-.049-1.791-.218-2.427-.465a4.902 4.902 0 01-1.772-1.153 4.902 4.902 0 01-1.153-1.772c-.247-.636-.416-1.363-.465-2.427-.047-1.024-.06-1.379-.06-3.808v-.63c0-2.43.013-2.784.06-3.808.045-1.064.218-1.504.344-1.857.182-.467.399.8.748 1.15.35.35.683.566 1.15.748.353.137.882.3 1.857.344 1.857.047 1.023-.058 1.351-.058 3.807v.468c0 2.456.011 2.784.058 3.807.045.975.207 1.504.344 1.857.182.466.399.8.748 1.15.35.35.683.566 1.15.748.353.137.882.3 1.857.344 1.054.048 1.37.058 4.041.058h.08c2.597 0 2.917-.01 3.96-.058.976-.045 1.505-.207 1.858-.344.466-.182.8-.398 1.15-.748.35-.35.566-.683.748-1.15.137-.353.3-.882.344-1.857.048-1.055.058-1.37.058-4.041v-.08c0-2.597-.01-2.917-.058-3.96-.045-.976-.207-1.505-.344-1.858a3.097 3.097 0 00-.748-1.15 3.098 3.098 0 00-1.15-.748c-.353-.137-.882-.3-1.857-.344-1.023-.047-1.351-.058-3.807-.058zM12 6.865a5.135 5.135 0 110 10.27 5.135 5.135 0 010-10.27zm0 1.802a3.333 3.333 0 100 6.666 3.333 3.333 0 000-6.666zm5.338-3.205a1.2 1.2 0 110 2.4 1.2 1.2 0 010-2.4z" clipRule="evenodd"></path>
                    </svg>
                  </a>
              ...
              <div>
                ...
                <ul className="space-y-4">
                  {['Features', 'Pricing', 'Demo', 'Security'].map((item) => (
                    <li key={item}>
                      <a href={`#${item.toLowerCase()}`} aria-label={item} className="text-blue-700 hover:text-blue-500 transition-colors">
                        {item}
                      </a>
                    </li>
                  ))}
              ...
              <div>
                ...
                <ul className="space-y-4">
                  {['Help Center', 'Contact', 'Status', 'Guides'].map((item) => (
                    <li key={item}>
                      <a href={`#${item.toLowerCase().replace(' ', '-')}`} aria-label={item} className="text-blue-700 hover:text-blue-500 transition-colors">
                        {item}
                      </a>
                    </li>
                  ))}
            ...
            <div className="flex flex-col md:flex-row justify-between items-center">
              ...
              <div className="flex space-x-10 mt-6 md:mt-0">
                {['Privacy', 'Terms', 'Cookies'].map((item) => (
                  <a key={item} href={`#${item.toLowerCase()}`} aria-label={item} className="text-blue-700 text-sm hover:text-blue-500 transition-colors">
                    {item}
                  </a>
                ))}
              </div>
